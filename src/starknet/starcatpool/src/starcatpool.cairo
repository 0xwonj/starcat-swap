...
#[starknet::interface]
pub trait IStarcatPool<TContractState> {
    ...
    fn swap_to_bitcoin(ref self: TContractState, token_in: ContractAddress, amount_in: u256, btc_address: ByteArray) -> u256;
    fn withdraw_to_bitcoin(ref self: TContractState, amount: u256, btc_address: ByteArray) -> u256;
}
...
pub mod StarcatPool {
    ...
    pub enum Event {
        ...
        SwapToBitcoin: SwapToBitcoin,
        WithdrawToBitcoin: WithdrawToBitcoin,
    }
    ...
    #[derive(Drop, PartialEq, starknet::Event)]
    pub struct SwapToBitcoin {
        #[key]
        pub caller: ContractAddress,
        pub token_in_address: ContractAddress,
        pub amount_in: u256,
        pub btc_address: ByteArray,
    }

    #[derive(Drop, PartialEq, starknet::Event)]
    pub struct WithdrawToBitcoin {
        #[key]
        pub caller: ContractAddress,
        pub amount: u256,
        pub btc_address: ByteArray,
    ...
    #[abi(embed_v0)]
    impl StarcatPool of super::IStarcatPool<ContractState> {
        ...
        fn swap_to_bitcoin(
            ...
            let (reserve0, reserve1): (u256, u256) = (self.reserve0.read(), self.reserve1.read());
            let (
                token_in, _, reserve_in, reserve_out
            ): (IERC20Dispatcher, IERC20Dispatcher, u256, u256) =
                if (is_token0) {
                (token0, token1, reserve0, reserve1)
            } else {
            ...
            // Here you would integrate with a Bitcoin bridge or service to handle the actual swap to Bitcoin
            // For now, we just emit an event

            self.withdraw_to_bitcoin(amount_out, btc_address_clone);

            self.emit(SwapToBitcoin {
                caller,
                token_in_address,
                amount_in,
                btc_address,
        ...
        fn withdraw_to_bitcoin(
            ...
            let total_supply = self.total_supply.read();
            let (amount0, amount1): (u256, u256) = (
                (amount * bal0) / total_supply, (amount * bal1) / total_supply
            );
            assert(amount0 > 0 && amount1 > 0, 'amount0 or amount1 = 0');

            self._update(bal0 - amount0, bal1 - amount1);

            // Here you would integrate with a Bitcoin bridge or service to handle the actual withdrawal to Bitcoin
            // For now, we just emit an event

            self.emit(WithdrawToBitcoin {
                caller,
                amount,
                btc_address,
            });
